...
namespace TJAPlayer3
{
	public class CScoreIni
	{
		...
		internal static double tゲーム型スキルを計算して返す( int nLevel, int nTotal, int nPerfect, int nCombo, EInstrumentPart inst, STAUTOPLAY bAutoPlay )
		{
			double ret;
		...
		internal static double t演奏型スキルを計算して返す( int nTotal, int nPerfect, int nGreat, int nGood, int nPoor, int nMiss, EInstrumentPart inst, STAUTOPLAY bAutoPlay)
		{
			if( nTotal == 0 )
				return 0.0;
		...
		internal static double dbCalcReviseValForDrGtBsAutoLanes( EInstrumentPart inst, STAUTOPLAY bAutoPlay )
		{
            //削除
			return 1.0;
		}
